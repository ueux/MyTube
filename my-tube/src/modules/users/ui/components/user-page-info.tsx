...
import { SubscriptionButton } from "@/modules/subscriptions/ui/componets/subscription-button"
...
export const UserPageInfo = ({ user }: UserPageInfoProps) => {
    ...
    return (
        <div className="py-6">
            <div className="flex flex-col md:hidden">
                <div className="flex items-center gap-3">
                    <UserAvatar size='lg' imageUrl={user.imageUrl} name={user.name} className="h-[60px] w-[60px]"
                        onClick={() => {
            ...
            <div className="hidden md:flex items-start gap-4">
                <UserAvatar size='xl' imageUrl={user.imageUrl} name={user.name} className={cn(userId == user.clerkId && "cursor-pointer hover:opacity-80 transition-opacity duration-300")}
                    onClick={() => {
                        if (user.clerkId === userId) {
                            clerk.openUserProfile();
                        }
